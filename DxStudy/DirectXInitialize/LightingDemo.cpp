...
void LightingDemo::UpdateScene(float dt)
{
	...
	VertexWithNormal* v = reinterpret_cast<VertexWithNormal*>(mappedData.pData);
	...
	mPointLight.Position.x = 70.0f*cosf(0.2f*mTimer.GetGameTime());
	mPointLight.Position.z = 70.0f*sinf(0.2f*mTimer.GetGameTime());
...
void LightingDemo::BuildLandGeometryBuffers()
{
	...
	std::vector<VertexWithNormal> vertices(grid.Vertices.size());
	...
	vbd.ByteWidth = sizeof(VertexWithNormal) * grid.Vertices.size();
...
void LightingDemo::BuildWaveGeometryBuffers()
{
	...
	vbd.ByteWidth = sizeof(VertexWithNormal) * mWaves->VertexCount();
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
