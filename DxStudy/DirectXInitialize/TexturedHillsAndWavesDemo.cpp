...
TexturedHillsAndWavesDemo::TexturedHillsAndWavesDemo(HINSTANCE hInstance)
...
void TexturedHillsAndWavesDemo::UpdateScene(float dt)
{
	...
	{
		v[i].Pos = mWaves->operator[](i);
		v[i].Normal = mWaves->Normal(i);

		// Derive tex-coords in [0,1] from position.
		v[i].Tex.x = 0.5f + mWaves->operator[](i).x / mWaves->Width();
		v[i].Tex.y = 0.5f - mWaves->operator[](i).z / mWaves->Depth();
	}
...
void TexturedHillsAndWavesDemo::DrawScene()
{
	...
	Effects::BasicTFX->SetDirLights(mDirLights);
	Effects::BasicTFX->SetEyePosW(mEyePosW);

	ID3DX11EffectTechnique* activeTech = Effects::BasicTFX->Light3TexTech;
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::BasicTFX->SetWorld(world);
		Effects::BasicTFX->SetWorldInvTranspose(worldInvTranspose);
		Effects::BasicTFX->SetWorldViewProj(worldViewProj);
		Effects::BasicTFX->SetTexTransform(XMLoadFloat4x4(&mGrassTexTransform));
		Effects::BasicTFX->SetMaterial(mLandMat);
		Effects::BasicTFX->SetDiffuseMap(mGrassMapSRV);
		...
		Effects::BasicTFX->SetWorldInvTranspose(worldInvTranspose);
		Effects::BasicTFX->SetWorldViewProj(worldViewProj);
		Effects::BasicTFX->SetTexTransform(XMLoadFloat4x4(&mWaterTexTransform));
		Effects::BasicTFX->SetMaterial(mWavesMat);
		Effects::BasicTFX->SetDiffuseMap(mWavesMapSRV);
